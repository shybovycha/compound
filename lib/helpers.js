...
HelperSet.prototype.stylesheetLinkTag = function stylesheetLinkTag() {
    ...
    mergeFiles(app, 'stylesheets', args).forEach(function(file) {
        ...
        links.push(genericTagSelfclosing('link', options, {
            href: href
        }));
    });
    return links.join('\n    ');
...
HelperSet.prototype.javascriptIncludeTag = function javascriptIncludeTag() {
    ...
    mergeFiles(app, 'javascripts', args).forEach(function(file) {
        ...
        scripts.push(helpers.tag('script', '', options, {
            src: href
        }));
    });
...
HelperSet.prototype.formTag = function(params, block) {
    ...
    if (!buf) {
        buf = arguments.callee.caller.caller.buf;
    }

    if (!buf) {
        buf = arguments.callee.caller.caller.caller.buf;
    }
        ...
        for (var prop in resource.errors) {
            if (resource.errors.hasOwnProperty(prop)) {
                ...
                resource.errors[prop].forEach(function (msg) {
                    out += h.tag('li', utils.camelize(prop, true) + ' ' + msg, {
                        class: 'error-message'
                    });
                });
...
HelperSet.prototype.getResourceName = function(resource) {
    return resource.modelName || (resource.constructor && resource.constructor.modelName) || resource.name || 'UnnamedResource' || false;
};
...
HelperSet.prototype.fieldsFor = function(resource, formParams, block) {
    ...
    var self = this;
    var resourceName = HelperSet.prototype.getResourceName(resource);
    var complexNames = (this.controller.app.set('view options') || {}).complexNames;
    var errorsFor = resource.errors_for || resource.errorsFor || {};
    ...
    /**
     * Gets the value of resource's field.
     * If name contains array-like expression - expands it.
     *
     * @param {String} name Field name
     * @returns {*} resource's field' value
     */
    function getResourceFieldValue(name) {
        if (/\[/.test(name)) {
            var part1 = name.replace(/\[.*/, '').replace(/^(.*?)$/, "'$1'").replace(/(\"|\')+/, '$1'),
                part2 = name.replace(/.*?(\[.*)/, '$1').replace(/\[([^\d]+)\]/, "['$1']"),
                result = null;

            try {
                result = eval("resource[" + part1 + "]" + part2);
            } catch (e) {
                console.error('Resource does not have field ', name);
            }
    ...
    function getError(field) {
        ...
        if (errorsFor[field]) {
            result = HelperSet.prototype.errorFor(makeId(field), errorsFor[field]);

            delete errorsFor[field];
        }
    ...
    var blockHelper = {
        ...
        input: function(name, params) {
            ...
            if (params.value === undef && params.type.toLowerCase() !== 'password') {
                var resourceValue = getResourceFieldValue(name);

                params.value = (typeof(resourceValue) !== 'undefined') ? resourceValue : '';
            }
            ...
            return getError(name) + HelperSet.prototype.inputTag({
                name: (params && params['name']) || makeName(name),
                id: (params && params['id']) || makeId(name)
            }, params);
        },
        checkbox: function(name, params) {
            ...
            if (params.value === undef) {
                params.value = getResourceFieldValue(name) || 1;
            }

            if (params.checked === undef) {
                if (getResourceFieldValue(name)) {
        ...
        label: function (name, caption, params) {
            if (typeof caption !== 'string') {
                ...
                var description = '';
                var model = resourceName || resource.constructor.modelName;
            ...
            return HelperSet.prototype.labelTag(
                caption,
                {
                    for: (params && params['for']) || makeId(name)
                },
                params);
        },
        submit: function(name, params) {
            return genericTag('button', name || 'Commit', {
                type: 'submit'
            }, params);
        },
        ...
        textarea: function (name, params) {
            var value = (params && params['value']) || getResourceFieldValue(name) || '';
            return getError(name) + genericTag('textarea', sanitizeHTML(value), {
                name: (params && params['name']) || makeName(name),
                id: (params && params['id']) || makeId(name)
            }, params);
        },
        ...
        select: function(name, options, params) {
            ...
            if (params.select === undef) {
                params.select = getResourceFieldValue(name) || '';
            }
            ...
            if (params.blank !== undef) {
                innerOptions += HelperSet.prototype.optionTag(sanitizeHTML(params.blank), {
                    value: ''
                })
            }
            ...
            options.forEach(function(option) {
                ...
                if (optionSelected && activeValue(option_value, optionSelected)) {
                    option.selected = 'selected';
                }

                // Generate the option Tags
                innerOptions += HelperSet.prototype.optionTag(option_name, option);
            });

            // Render the select
            return getError(name) + HelperSet.prototype.selectTag(innerOptions, {
                name: makeName(name),
                id: makeId(name)
            }, params);
        }
...
HelperSet.prototype.formFor = function formFor(resource, params, block) {
    ...
    if (resource) {
        ...
        if (!params.action) {
            params.action = this.controller.app.compound.map.pathTo[utils.underscore(HelperSet.prototype.getResourceName(resource))](resource);
        }
    }
...
HelperSet.prototype.formTagBegin = function(params) {
    ...
    if (_method !== params.method) {
        html += HelperSet.prototype.inputTag({
            type: "hidden",
            name: "_method",
            value: _method
        });
...
HelperSet.prototype.form_tag_begin = HelperSet.prototype.formTagBegin;
...
HelperSet.prototype.form_tag_end = HelperSet.prototype.formTagEnd;

HelperSet.prototype.errorFor = function(fieldName, errorMessage) {
    return '<div class="error-message" for="' + fieldName + '">' + errorMessage + '</div>';
};

HelperSet.prototype.error_for = HelperSet.prototype.errorFor;
...
HelperSet.prototype.selectTag = function(innerOptions, params, override) {
    if (params['blank']) {
        innerOptions = HelperSet.prototype.optionTag(params['blank'], {
            value: ''
        }, {}) + innerOptions;
 ...
 * Result:
 *
 *     <input type="radio" name="name" value="value" params />
 *
 * @methodOf HelperSet.prototype
 ...
 * @returns {String} Finalized select tag
 */
HelperSet.prototype.radioTag = function(name, value, params, override) {
    params = params || {};
    params['type'] = 'radio';
    params['name'] = name;
    params['value'] = value;

    return genericTag('input', '', params, override);
};
 ...
 * Result:
 *
 *     <input type="checkbox" name="name" value="value" params />
 *
 * @methodOf HelperSet.prototype
 * @author [Artem Shoobovych](https://github.com/shybovycha)
 * @param {String} name Input' "name" attribute
 ...
 * @returns {String} Finalized select tag
 */
HelperSet.prototype.checkboxTag = function(name, value, params, override) {
    params = params || {};
...
HelperSet.prototype.checkbox_tag = HelperSet.prototype.checkboxTag;
...
function genericTag(name, inner, params, override) {
    return html('<' + name + htmlTagParams(params, override) + '>' + sanitizeHTML(inner) + '</' + name + '>');
}
...
function checkFile(app, type, file) {
    var isExternalFile = regexps.isHttp.test(file),
        isAbsolutePath = regexps.isAbsolutePath.test(file),
    ...
    return (isAbsolutePath && file) || (isExternalFile ? href : appprefix + href);
};
