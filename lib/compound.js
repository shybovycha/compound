...
function staticHandler(req, res, next) {
    var path = require('path'),
        url = require('url'),
        fs = require('fs-extra'),
        static_send = require('send');

    var request_partial_path = url.parse(req.url).pathname,
        request_full_path = path.join(process.cwd(), url.parse(req.url).pathname),
        request_path_stats = fs.existsSync(request_full_path) && fs.statSync(request_full_path),
        request_path_exists = request_path_stats && request_path_stats.isFile(),
        $ = compoundUtils.stylize.$,
        log = compoundUtils.debug;

    if (request_path_exists) {
        log($('>>> Serving static file ', 'cyan').bold + '`{0}`'.format(request_partial_path) + $(' -> ', 'cyan') + '`{0}`'.format(request_full_path));

        return static_send(req, request_partial_path).root(process.cwd()).pipe(res);
    }
